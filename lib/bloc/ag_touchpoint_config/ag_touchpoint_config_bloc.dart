...
class AGTouchpointConfigBloc
    extends Bloc<AGTouchpointConfigEvent, AGTouchpointConfigState> {
  ...
  void _onInit(AGTouchpointConfigEventInit event,
      Emitter<AGTouchpointConfigState> emit) async {
    emit(AGTouchpointConfigLoading());

    try {
      final config = await contentApi.getAGTouchpointConfigForTouchpoint(
  ...
  void _onUpdateConfig(AGTouchpointConfigEventUpdateConfig event,
      Emitter<AGTouchpointConfigState> emit) async {
  ...
  void _onUpdateContent(AGTouchpointConfigEventUpdateContent event,
      Emitter<AGTouchpointConfigState> emit) async {
    if (state is AGTouchpointConfigLoaded) {

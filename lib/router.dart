...
// import 'package:enter_cms_flutter/routes/content_page.dart';
...
//     log.fine('Going to ${state.location}, query: ${state.queryParameters}, all: ${state.queryParametersAll}');
//     final loggedIn = authBloc.state is AuthSuccess;
//     final loggingIn = state.location == '/login';
//
//     // bundle the location the user is coming from into a query parameter
//     final fromp = state.location == '/' ? '' : '?from=${state.location.replaceAll('&','\$')}';
//     if (!loggedIn) return loggingIn ? null : '/login$fromp';
//
//     // if the user is logged in, send them where they were going before (or
//     // home if they weren't going anywhere)
//     if (loggingIn) return state.queryParameters['from']?.replaceAll('\$', '&') ?? '/';
//
